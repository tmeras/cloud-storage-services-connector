            ...
            if file_size > 30 * MB:
                try:
                    ...
                    if file_id is None:
                        ...
                        uploader = self.client.folder(folder_id).create_upload_session(file_size = file_size, file_name = localdir.split('/')[-1])
                    else:
                    ...
                    utils.print_string("Could not upload '{}' in chunks: {}".format(localdir,e),utils.PrintStyle.ERROR)
                    sys.exit()
